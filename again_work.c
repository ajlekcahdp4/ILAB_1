...
double epsilon_double_check(const double coef, double number);
...
double scan_coef (char coef)
{
    ...
    do {
        ...
        clear_buffer();//new function void clear_buffer
    } while (scan_check == 0);
    ...
    Discriminant = coef_b*coef_b - 4*coef_a*coef_c;
        if (Discriminant > 0.0)
        {
            ...
            *x2 = (-coef_b + discriminant_root)/(2*coef_a);
            ...
            if (epsilon_double_check (Discriminant, 0)) 
                *x1 = -coef_b/(2*coef_a);    
        }
...
double epsilon_double_check(double dbl1, double dbl2)
{
    double epsilon = 1e-4; 
...
void solve_equation(double coef_a, double coef_b, double coef_c, double *x1, double *x2) 
{
    if (epsilon_double_check (coef_a, 0)) //== for floats and doubles
